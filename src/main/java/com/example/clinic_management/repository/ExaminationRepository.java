...
public interface ExaminationRepository extends JpaRepository<ExaminationDetail, Long> {
    ...
    @Query(
            "SELECT e FROM ExaminationDetail e WHERE e.examinationType IN :labTests AND SIZE(e.imagesTest) = 0 AND e.createdAt = :createdAt ORDER BY e.id")
    ...
    @Query(
            "SELECT e FROM ExaminationDetail e WHERE e.examinationType = :examinationType AND SIZE(e.imagesTest) = 0 AND e.createdAt = :createdAt ORDER BY e.id")
